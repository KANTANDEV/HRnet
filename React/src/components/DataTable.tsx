	...
	return (
		<div className='table_container'>
				...
				<thead className='bg-gray-50'>
					<tr>
						{columns.map((column, index) => (
							<th key={index} className={'p-3 text-left cursor-pointer ' + (sorting.column === column.data ? (sorting.type === 'asc' ? 'sorting_asc' : 'sorting_desc') : 'sorting')} onClick={() => handleSorting(column.data)}>
								{column.title}
							</th>
						))}
					</tr>
					...
					{filteredData.length ? (
						filteredData.slice((pageNb - 1) * showNb, (pageNb - 1) * showNb + showNb).map((data, index) => (
							<tr key={index} className={index % 2 ? 'bg-gray-50' : ''}>
								{columns.map((column, index) => (
									<td key={index} className={'p-3 ' + (sorting.column === column.data ? 'sorting' : '')}>
										{data[column.data]}
									</td>
								))}
							</tr>
			...
			<div className='table_paginate'>
				<a className={'px-3 py-1 rounded border mr-2 ' + (pageNb === 1 ? 'bg-gray-200 cursor-not-allowed' : 'bg-white cursor-pointer')} onClick={() => handlePageChange('prev')}>
					Previous
				</a>
				<span>
					{filteredData.length
						? new Array(maxPage).fill(0).map((data, page) => (
								<a key={page + 1} className={'px-3 py-1 rounded border mr-2 ' + (pageNb === page + 1 ? 'bg-blue-600 text-white cursor-not-allowed' : 'bg-white cursor-pointer')} onClick={() => handlePageNumChange(page + 1)}>
									{page + 1}
								</a>
						  ))
						: null}
				</span>
				<a className={'px-3 py-1 rounded border mr-2 ' + (pageNb >= maxPage ? ' disabled' : '')} onClick={() => handlePageChange('next')}>
					Next
				</a>
			</div>
		</div>
